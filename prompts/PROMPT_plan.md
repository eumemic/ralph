0a. Study `${SPECS_DIR}/*` with up to 250 parallel subagents to learn the application specifications.
...
1. Study `${PLAN_FILE}` (if present; it may be incorrect) and use up to 500 subagents to study existing source code and compare it against `${SPECS_DIR}/*`. Analyze findings, prioritize tasks, and create/update `${PLAN_FILE}` as a bullet point list sorted by priority of items yet to be implemented. Consider searching for TODO, minimal implementations, placeholders, skipped/flaky tests, and inconsistent patterns. Keep the plan up to date with items considered complete/incomplete.
